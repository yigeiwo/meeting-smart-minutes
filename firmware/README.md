...
> ⚠️ **重要：请使用最新固件**
> 本目录下的 `.bin` 是**预编译快照**。若你刚更新过 `ai-passport/` 源码（例如新增了
> `main/card_link.c` 的 5566 TCP/NDJSON 桥接客户端），必须**重新编译并替换**这些 bin 再烧录，
> 否则胸卡仍运行旧固件：
> ```bash
> cd ai-passport
...
* **上行（卡片 → 工作台）**：`hello` 握手、`battery` 电量、`record_start` / `record_stop`
  录音启停、`ping` 心跳、`combo_menu` 返回菜单，以及 **`audio` 真实录音音频流**
  （16kHz / 16bit / 单声道 PCM，base64 封装，每帧 1024 字节）。
...
* **完整闭环**：胸卡按 OK 开始录音 → 音频实时推给工作台并落盘为 `records/card_*.wav`
  ...
  → 结果回推胸卡 → 屏幕第 1 页显示真实纪要标题与云文档状态，第 3 页显示真实待办清单。
* **真实错误透传**：链路断开、工作台提炼失败、超过 180 秒未返回，胸卡屏幕都会如实显示
  对应原因，**不会伪造"已同步成功"**。
* **中文字库**：固件内置 `font_chinese_14`（14px / 4bpp / 黑体）已扩容为 **GB2312 全集**
  （6763 个汉字 + 682 个符号 + ASCII，共 7541 个字形，约占用 815KB Flash）。
  会议纪要中的常规中文（含"条、品、钟、但、嵌、员"等此前会留空的字）均可正常显示。
