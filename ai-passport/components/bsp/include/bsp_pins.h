...
//
// ⚠ 上键的下限必须是 0,不能留"死区":
//   iot_button 的判定是【闭区间】—— button_adc.c 里 `vol <= max && vol >= min`。
//   上键是 0Ω 直通地,按下读到约 0mV; 若把下限写成 20(想躲干扰),0~19mV 就落在所有
//   窗口之外,被当成"没有按键",表现为【上键完全没反应】,而下键/确定键一切正常
//   (它们离 0 很远,不受影响)。松开态是 3300mV,不存在误触风险,这个死区没有必要。
// 上键: ~0mV   (0~140mV)
// 下键: ~300mV (180~430mV)
// 确定: ~595mV (460~1200mV)
