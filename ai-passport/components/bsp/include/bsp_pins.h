...
//
// ⚠ 判定是【闭区间】(button 组件里 `vol <= max && vol >= min`), 且上键的下限必须是 0:
//   上键是 0Ω 直通地, 按下读到约 0mV; 若把下限写成 20/40(想躲干扰), 0~19mV 就落在所有
//   窗口之外, 表现为【上键完全没反应】—— 历史上这样回归过两次。
//   但下限设成 0 会引入另一个坑: 本板空闲电平在 ADC 12dB 量程上限, 该工作点偶发
//   raw=0 的坏读数, 折算成 0mV 会被当成"上键按下", 表现为【页面自己跳回主菜单】。
//   这一点由 bsp_button.c 的独立采样器 + bsp_button_filter.h 的"连续 3 格同窗口"门控
//   解决, 不要靠调这组窗口来回避 —— 窗口只描述电平, 连续性由滤波器负责。
// 上键: ~0mV   (0~140mV)
// 下键: ~300mV (180~430mV)
// 确定: ~595mV (460~1200mV)
// 松开: 3300mV (>1200mV 全部视为松开无按键)
#define BSP_BTN_MV_TABLE  { {0, 140}, {180, 430}, {460, 1200} }
