...
- 修复上键自发乱按、页面自动跳回主菜单：本板空闲电平位于 `ADC_ATTEN_DB_12` 量程上限，
  实测该工作点会偶发 raw=0 的坏读数（10 ms 采样下从不连续出现 2 格），折算 0 mV 后落进
  上键窗口，被按键组件当成反复按下。现在由 `bsp_button.c` 的 10 ms 独立采样器 +
  `bsp_button_filter.h` 的「连续 3 格同窗口」在事件出口做门控，纯逻辑由
  `tests/test_button_filter.c` 离线覆盖；同时把上键窗口下限恢复为 0（`{0,140}`），
  并在硬件指南记录该工作点、坏读数形态与数字电平不可用作交叉判据的原因。
