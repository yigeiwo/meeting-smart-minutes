...
| --- | ---: | ---: |
| UP | 约 0 mV | `[0, 140]` mV |
| DOWN | 约 300 mV | `[180, 430]` mV |
| OK | 约 595 mV | `[460, 1200]` mV |
| 松开 | 约 3300 mV | 不属于任何按键窗口 |

绝不能用约 45 kΩ 且离散性大的 ESP32-C3 内部上拉替代 10 kΩ 外部上拉，否则三个档位会挤在低电压区并受温漂影响。

⚠ 上表边界是**闭区间**：button 组件的 `button_adc.c` 判定为 `vol <= max && vol >= min`。UP 键经 0 Ω 直接接地，实测约 0 mV，因此 **UP 窗口的下限必须是 0**。历史上把下限写成 20 或 40（想躲干扰）导致过两次“上键完全无反应、其余两键正常”的回归（见 `恢复0mV按键阈值` 那次修复），改动阈值时不要重犯：按下电压几乎为 0，松开态却是 3300 mV，这个死区既无必要也无收益。

⚠ 但下限设成 0 之后还有第二个坑，实测结论如下（这是本页最重要的一条硬件事实）：

| 实测项 | 结果 |
| --- | ---: |
| 空闲（无按键）ADC 原始值 | raw **恒为 4095**，即满量程；引脚被 10 kΩ 上拉到 3.3 V，已到 `ADC_ATTEN_DB_12` 量程上限 |
| 偶发坏读数 | raw **= 0**（折算 0 mV），10 ms 采样下**从不连续出现 2 格**，频率约每秒 1~3 次 |
| 坏读数落点 | 恰好落在 UP 窗口内，因此**只有上键会中招**：UP 被当成反复按下，DOUBLE 事件不断触发“返回主菜单”，表现为**页面自己跳回主菜单、无法使用** |
| `gpio_get_level(GPIO0)` | 该引脚一旦配置为 ADC 输入，**数字输入通路即被关闭**，读数恒为 0，与 ADC 的 4095 矛盾 —— 所以数字电平**不能**用作交叉判据 |

对策不是去调窗口（窗口只描述电平，改窗口要么让上键失效、要么让坏读数继续生效），而是在 BSP 层加一道**连续性门控**：

- `bsp_button.c` 用 10 ms 周期的独立采样器维护每个键的滤波状态（同时缓存电压给 Button 页，避免再开第二个 ADC 读者）。
- `bsp_button_filter.h` 规定**连续 3 格（30 ms）同窗口**才算真实按下；真人按键 ≥100 ms（≥10 格），与门限有 3 倍以上裕量，而单格坏读数永远凑不满 3 格。
- 按键组件报来的事件只有在 800 ms 内存在过“确认按下”时才放行，其余事件被丢弃并限频告警。
- 该逻辑是纯函数，`tests/test_button_filter.c` 用真实抓包形态（孤立单格坏读数）离线验证，`tools/validate.sh --static` 会跑。

实现上的关键限制：

- BSP 先创建唯一的 ADC1 oneshot unit，再把同一句柄交给三个 `iot_button` ADC 设备；采样器与 `bsp_button_read_mv()` 复用它。不要为电压显示另建 ADC1 unit，也不要再开第三个 ADC 读者。
- ADC 衰减为 `ADC_ATTEN_DB_12`，必须与依赖的 button 组件内部配置保持一致。升级组件后要重新核对。
- ADC 校准句柄创建失败会让采样器无法判键，此时 `bsp_button_init()` 直接返回 `ESP_FAIL`（整块板子按键会全部被门控挡掉，硬失败比静默失灵好排查）。
- 按键采样定时器启动失败同样是硬失败，理由同上。
- 回调来自 button 组件的定时器任务，不能阻塞、录音、播放或直接做重 UI 操作。
- 事件包括 PRESS、CLICK、DOUBLE、LONG。应用菜单主要消费 CLICK；页面中的 OK LONG 被全局拦截用于返回。
- 排查“某个键自发乱跳”时，把 `bsp_button.c` 的 `BSP_BTN_DIAG` 改成 1：它与采样共用同一次读取，会每 2 秒打印一行 10 ms/格的分类波形和“最长连续 U 格数”，可直接看出坏读数是孤立 1 格还是连续多格。
...
| 上键自发乱按 / 页面自动跳回主菜单 | 空闲电平在量程上限导致的偶发 raw=0 坏读数；确认 `bsp_button` 的采样定时器在跑、`BSP_BTN_FILTER_SAMPLES` 未被改小 |
| 三个按键全部无反应 | 采样器没起来（校准或定时器创建失败会在 `bsp_button_init` 硬失败并打错误日志），事件全被门控挡掉 |
